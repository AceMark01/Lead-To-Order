...
function NewFollowUp() {
  ...
  const [formData, setFormData] = useState({
    ...
    description: "", // Description field for Leads Tracker column AE
  })
  ...
  const [prefilledNob, setPrefilledNob] = useState("")
  ...
  // Function to fetch lead details including NOB from FMS sheet
  const fetchLeadDetails = async (leadId) => {
    try {
      ...
      if (data && data.table && data.table.rows) {
        // Find row where column B (index 1) matches leadId
        const leadIdStr = String(leadId).trim()
        console.log("Looking for leadId:", leadIdStr)

        const leadRow = data.table.rows.find(row => {
          if (row.c && row.c[1]) {
            const rowLeadId = String(row.c[1].v).trim()
            console.log("Checking row with leadId:", rowLeadId)
            return rowLeadId === leadIdStr
          }
          return false
        })

        console.log("Found leadRow:", leadRow ? "Yes" : "No")

        if (leadRow) {
          // Get NOB from column U (index 20)
          const nobValue = leadRow.c[20] ? leadRow.c[20].v : ""
          console.log("NOB value from column U (index 20):", nobValue)

          if (nobValue) {
            setPrefilledNob(nobValue)
            console.log("Set prefilledNob to:", nobValue)
          } else {
            console.log("NOB value is empty for this lead")
          }
        } else {
          console.log("No matching row found for leadId:", leadIdStr)
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (enquiryStatus === "expected") {
        ...
        // Add description in column AE (index 30)
        rowData.push(formData.description || "")
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      value={prefilledNob}
                      readOnly
                      placeholder="NOB will be auto-filled"
                    />
                  </div>
